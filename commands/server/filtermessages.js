...
module.exports = {
    ...
    usage: `\`${Prefix}filter <create | text | link | role | delete | collection>\``,
    category: `Server`,
    accessibly: `Administrator`,
    aliases: ['filter', "filtermessages"],
    cooldown: 10,
    details: `[filter](${CommandsHyperlink} 'Enable/Disable filter messages. If filter text/link enabled in channel, no one allowed to send text/link messages except whitelisted roles.')`,
    ...
    launch: async(client, message, args) => {
        try {
            ...
            if (option === 'create') {
                ...
                const embed = new Discord.MessageEmbed()
                .setColor("GOLD").setTitle("Message Filter - Info").setTimestamp()
                .addField("⚒️ | Commands", 
